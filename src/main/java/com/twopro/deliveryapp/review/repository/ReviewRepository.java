...
public interface ReviewRepository extends JpaRepository<Review, UUID> {
    ...
    @Query("select r from Review r where r.order.id =:orderId and r.deletedBy is null")
    Optional<Review> findFetchByOrderIdAndDeletedAtIsNull(UUID orderId);

    // 조인패치로 리뷰->회원 가져오기, 조인패치로 리뷰->오더->오더메뉴->메뉴로 한번에 가져오기
    @Query("select r from Review r join fetch r.user u join fetch r.order o join fetch o.store join fetch o.orderItems oi join fetch oi.menu where r.id =:reviewId and r.deletedBy is null")
    ...
    @Query("select r from Review r join fetch r.user u join fetch r.order o join fetch o.orderItems oi join fetch o.store join fetch oi.menu where r.deletedBy is null and o.user.userId =:userId")
